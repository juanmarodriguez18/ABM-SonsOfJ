...
export async function guardarPedidoEnBD(pedido: Pedido): Promise<void> {
  try {
    ...
    throw new Error(
      "Error al guardar el pedido. Por favor, inténtalo de nuevo más tarde."
    );
  }
}
...
export const facturarPedido = async (pedido: Pedido, email: string) => {
  try {
    ...
    const response = await fetch(
      `http://localhost:8080/facturacion/emitir?emailCliente=${email}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const descargarFactura = async (pedido: Pedido) => {
  const response = await fetch(
    `http://localhost:8080/facturacion/factura/${pedido.id}`
  );
